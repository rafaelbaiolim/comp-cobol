...
void montarMatrizProducao(){
	...
	{   
	    ...
	    if (ptr != NULL)
	    {
	    	...
	    	strncpy(auxExpressao, producaoAUX[nroProducao],2);
			strncat(auxProducao, &producaoAUX[nroProducao][2], strlen(&producaoAUX[nroProducao][2]));
	    	
			token = strtok(auxProducao, "|");
	    	while (token != NULL)
	    	{	    		
	    		//strncat(matrizProducao[nroProducao], auxExpressao, 50 - strlen(matrizProducao[nroProducao]) -1);
	    		//strncat(matrizProducao[nroProducao], token, 50 - strlen(matrizProducao[nroProducao]) -1);
